...
class RaceResultItem {
  ...
  // Returns the segment time for a given segment or null if not available
  SegmentTime? getSegmentTime(String segmentName) {
    return segmentTimes[segmentName];
  }

  // Returns the formatted total duration or "--:--:--" if not available
...
class RaceResultBoard {
  ...
  static RaceResultBoard createFromData({
    ...
    // Sort by total duration (completed participants first, then by fastest time)
    items.sort((a, b) {
